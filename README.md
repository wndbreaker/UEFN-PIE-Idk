UEFN-PIE (credits to .coltonis for this thingy bc i didnt have it and also wildfurball4 for main stuff ig)
...
Download 38.11.
Download UEFN for 38.11.
Download UEFN-PIE's code as a ZIP and extract it to the same location your Fortnite folder (merging FortniteGame folders).
Download xdelta and set "chapter7endeventpie_Exe.xdelta" as the patch and UnrealEditorFortnite-Win64-Shipping.exe in "FortniteGame/Binaries/Win64" as the Source File, and after that then set "chapter7endeventpie_Engine.xdelta" as the patch and UnrealEditorFortnite-Engine-Win64-Shipping.dll in "FortniteGame/Binaries/Win64" as the Source File, and then set "chapter7endeventpie_Common.xdelta" as the patch and UnrealEditorFortnite-Common-Win64-Shipping.dll in "FortniteGame/Binaries/Win64" as the Source File.
make sure you rename the patch files to its original names!!
Download Fiddler Classic, once Fiddler is set up copy the contents of FiddlerScript.txt or the port 3551 version (i recommend using just the regular neonite) and paste it into the FiddlerScript tab inside Fiddler, go into Tools -> Options -> HTTPS and make sure Capture HTTPS CONNECTs, Decrypt HTTPS traffic, Ignore server certificate errors (unsafe) are all checked, then click Actions and Trust Root Certificate clicking yes when prompted.
Download Neonite, and run run.bat.
Make a shortcut to the patched exe you just created with the arguments "-disableplugins=ValkyrieFortnite -enableplugins=ValkyriePIE", then go to the path "%localappdata%\UnrealEditorFortnite\Saved\Config\WindowsEditor" copy the Engine.ini to that location, then make it read only by right clicking on the file, -> properties > attributes: read only.
Open your patched exe from xdelta, click on Edit in the top left, then Editor Preferences..., scroll down to Play Credentials and Enable Logins and add Credentials, User Id will be your username on Neonite, Password doesn't matter but needs to be filled in, Type should be set to epic or exchangecode.
In UEFN Click the three dots next to Platform and scroll down to NetMode, make sure it is set to Play Standalone and click play, if it loads into lobby, try again.
